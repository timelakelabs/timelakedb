...
- **Cluster-wide Grafana dashboard** shipped 2026-08-28 (#117): the deep,
  ...
  **Prometheus** scraping every node's `/metrics` relabelled by node+role,
  ...
  `docs/evidence/cluster-dashboard-drill.log` (15/15). #123 (2026-08-28)
  closed the one caveat: the ingesters now expose `catalog_head` too, so the
  convergence head spread spans the write front, not just the followers.
