---
name: tldb-gate
description: Build, test, and run TimelordDB's benchmark gate cheaply — the exact commands and pass criteria, so a fresh session needs no re-derivation. Use for "run the gate", "build and test", "verify milestone".
---

# TimelordDB gate runner
...
```powershell
docker run --rm -v "C:\project-time-lord-db\TimelordDB:/src" -v timelord-cargo-cache:/usr/local/cargo/registry -w /src rust:1-slim sh -c "cargo test --workspace 2>&1 | grep -E 'test result|error|FAILED' | grep -v 'ok. 0 passed'"
```

## Rebuild + start (with Grafana fixture)

```powershell
cd C:\project-time-lord-db\TimelordDB\bench
docker compose -f compose/timelorddb.yml --profile grafana up -d --build
# fresh data: down first, then Remove-Item -Recurse compose\data\timelord
```
...
```powershell
cd C:\project-time-lord-db\TimelordDB\bench
python bench.py run --backend timelorddb --scale smoke  --label <X>   # shakeout, ~15 s
python bench.py run --backend timelorddb --scale laptop --label <X>   # milestone gate, ~1 min
```
...
- Grafana (port 3003, admin/admin): datasource health OK; funnel panel
  query returns 10 steps. Crash drill: `docker restart -t 0 timelorddb`
