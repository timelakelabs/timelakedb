#!/bin/sh
# Build the .deb and .rpm release artifacts.
#
...
#
# A dynamically linked binary inherits the glibc of the machine that linked
# it, and refuses to start on anything older. Linked on a current image the
# server demands GLIBC_2.39, which RHEL 9 (2.34), Debian 12 (2.36) and Ubuntu
# 22.04 (2.35) do not have — the package would install cleanly and then fail
# with a symbol-lookup error, which is the worst way to learn this. Building
# on Debian 11 puts the floor at glibc 2.31 and covers RHEL/Rocky 9+, Debian
# 11+ and Ubuntu 20.04+. Raise BUILD_IMAGE only with that trade-off in mind.
#
...
TARGET_VOLUME=${TARGET_VOLUME:-tl-portable-target}
...
echo "building timelakedb $PKG_VERSION ($PKG_ARCH, glibc >= $PKG_GLIBC_MIN)"

mkdir -p dist

if [ "$SKIP_BUILD" -eq 0 ]; then
    echo "==> compiling the server on $BUILD_IMAGE"
    docker run --rm \
        -v "$REPO_ROOT":/w \
        ...
        "$BUILD_IMAGE" sh -c '
            ...
            cargo build --release -p timelake-server
            # Symbols are ~150 MB of the artifact and nothing debugs from a
            # package build; a separate debuginfo package is future work.
            strip /target/release/timelake-server
            cp /target/release/timelake-server /w/dist/timelake-server
        '
else
    echo "==> --skip-build: reusing dist/timelake-server"
    [ -f dist/timelake-server ] || { echo "dist/timelake-server missing"; exit 1; }
fi
...
# build instead of breaking somebody's server.
...
FLOOR=$(docker run --rm -v "$REPO_ROOT":/w -w /w "$BUILD_IMAGE" sh -c '
    ...
    objdump -T dist/timelake-server | grep -o "GLIBC_[0-9.]*" | sort -V | tail -1
' | tr -d '\r')
